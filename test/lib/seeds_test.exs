defmodule Hpd.SeedsTest do
  ...
  test "get_data_files lists all CSV files in the data directory", context do
    ...
    assert Seeds.get_data_files(context[:test_data_dir]) == [file_2, file_1]
  end
  ...
  test "decode_file decodes a file to a list of rows", context do
    ...
    assert assert Seeds.decode_file(path) == expected
  end
  ...
  test "insert_system inserts a valid system into the database" do
    ...
    assert capture_io(fn -> Seeds.insert_system(@valid_attrs) end) =~ "Inserted system some systemName for company some companyName."
  ...
  test "insert_system does not insert an invalid system into the database" do
    count_before = system_count()
    assert capture_io(fn -> Seeds.insert_system(@invalid_attrs) end) =~ "Error inserting the following system:"
    count_after = system_count()
